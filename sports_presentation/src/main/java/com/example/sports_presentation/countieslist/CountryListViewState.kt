...
sealed interface CountryListViewState : ViewState {
    object Loading : CountryListViewState     // An object is a singleton instance of a class, and in this case, it represents the loading state of the view. Since loading is a single state and doesn't require any additional data, it can be represented as an object.
    class Success(countriesList: List<CountryModel>) : CountryListViewState  // This class represent the success state of the view. It can hold additional data such as the list of countries in the case of Success .
    class Error(errorMessage: String) : CountryListViewState
}
